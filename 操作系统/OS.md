...
进程是分配资源的单位(包括内存的地址空间), 因此各进程拥有的内存地址空间相互独立
...
- 基于存储区的共享:操作系统在内存中划分出一块共享存储区域, 数据的形式, 存储位置都有通信进程控制, 而不是操作系统.这种共享方式速度很快, 是一种高级通讯方式
- 基于数据结构的共享:比如共享空间只能放一个长度为10的数组. 这种共享方式速度慢,限制多,是一种低级通讯方式

## 消息传递

进程间的数据会以`格式化的消息`为单位.通过系统提供的"发送消息/接受消息"两个原语进行数据交换
...
- 直接通信方式: 消息发送要指明接受进程的ID
  进程的PCB管理进程的消息队列, 进程调用发送消息原语将消息发送到系统, 另一个进程通过调用接受原语接受消息
- 间接通信方式: 通过"信箱"间接地通信, 因此又称为"信箱通信方式"
  以"邮箱"作为中间 , 进程通过系统调用申请一个邮箱, 发送原语指明发送的信箱, 往信箱发送消息, 另一个进程指明发送接受原语,指明要接受的信箱

## 管道通讯

管道的通讯是单项的

是一个特殊的共享文件, pipe文件. 其实就是 一个大小固定的`内存缓冲区`

通过系统调用申请一个管道文件, 数据读写是具有先进先出的, 和共享存储的区别就是先进先出的特性(本质是一个循环队列), 而共享内存可以从任意位置读写

- 管道只能采用半双工通信, 某一时间段内只能实现单项的传输. 若果要实现双向同时通信,则需要设置两个管道
- 各个进程要互斥地访问管道(由操作系统实现)
- 当管道写满时, 写进程将被阻塞, 直到读进程将管道中的数据取走, 即可唤醒写进程
- 当管道被读空, 读进程将被阻塞, 直到写进程往管道中写入数据, 即可唤醒读进程
- 管道中的数据一旦被读出, 就彻底的消失, 因此有多个读进程同时读一个管道的时候, 可能会错乱
  解决方案
  - 一个管道允许多个写进程, 一个读进程(考试标准答案)
  - 多个写进程, 多个读进程, 但是系统会让各个读进程轮流的从管道中读取数据(Linux方案)
...
- 线程也有就绪, 阻塞, 运行三种基本状态
- 线程几乎不拥有系统资源
- 同一个进程的不同线程间共享进程的资源(IO设备, 内存地址空间)
- 由于共享内存地址空间, 同一个进程中的线程间通信甚至无需系统干预
